...
class Speaker extends Component {
	...
	deleteSpeaker = () => {
		...
		cloneSpeakers.splice(
			Number(
				this.props.speaker.speakers.findIndex((el) => {
					return el.id === this.props.match.params.speakerId
				})
			),
			1
		)
				...
				<div className="row">
					{this.state.image ? (
						<div className="four columns">
							<div
								className="resim noTouch"
								width="100%"
								style={{
									backgroundImage:
										"url('https://app.sliconf.com/api/image/get/" + this.state.image + "')",
									float: 'left'
								}}
							/>
						</div>
										...
										<div
											...
											onClick={() => {
												this.props.history.push(
													'/events/' + this.props.match.params.eventId + '/talks'
												)
											}}
											className="room"
											style={{ background: 'gainsboro', margin: '0 5px 5px 0' }}
										>
											{topic}
										</div>
									))}
								...
								<div className="twelve columns">
									...
									<p>
										{this.state.twitter ? (
											...
											>
												<LogoTwitter fontSize="32px" color="#1da1f2" />
											</a>
										) : (
											''
										)}{' '}
										{this.state.linkedin ? (
											...
											>
												<LogoLinkedin fontSize="32px" color="#0077b5" />
											</a>
										) : (
											''
										)}
									...
									onClick={() => {
										this.props.history.push(
											'/events/' +
												...
												'/editspeaker/' +
												this.props.match.params.speakerId
										)
									}}
								>
									EDIT SPEAKER
